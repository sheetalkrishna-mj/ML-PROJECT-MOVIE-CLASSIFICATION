...
differentLearningRates = [0.1]
differentRegParams = [10000] 
foldArray = [0, 1, 2, 3, 4]
seed = 0
...
opFileName = "Logistic_Predictions.csv"
epochsForQ1 = 10
epochsForQ2 = 20
...
def trainSVM(curEpoch, matrix, trainLbls, wtVector, innerIndexes):
    if curEpoch == 1:
        ...
        for innerRate in differentLearningRates:
            ...
            for innerRegParam in differentRegParams:
                ...
                wtVector[innerRate][innerRegParam] = numpy.random.uniform(low=-1, high=1, size=(highestFeatureValue + 1))  # numpy.zeros(highestFeatureValue + 1)
    for rowNumber in innerIndexes:
        ...
        for innerRate in differentLearningRates:
            ...
            for innerRegParam in differentRegParams:
                try:
                    x = trainLbls[rowNumber] * wtVector[innerRate][innerRegParam].dot(dummyRow)
                    wtVector[innerRate][innerRegParam] = wtVector[innerRate][innerRegParam] + (
                            (newRate * trainLbls[rowNumber] * dummyRow) /
                            float(1 + math.exp(x))) - ((2 * newRate * wtVector[innerRate][innerRegParam]) / float(innerRegParam))
                except:
                    continue
        ...
        for rowNumber in matrix:
            ...
            for innerRate in differentLearningRates:
                for innerRegParam in differentRegParams:
                    ...
                    #predictedLabel = 1 if (numpy.sum(wtVector[innerRate][innerRegParam][indexes]*.dot(dummyRow)) > 0) else -1
...
def getWV(innerFoldCount):
    print("Getting weight vector for Cross-Validation combi: " + innerFoldCount)
...
print('The best learning rate after CV for Logistic Regression is: ' + str(bestLearningRateForSvm) +
      ' and best Sigma is: ' + str(bestRegParamForSvm) +
...
print('Epoch at which Logistic Regression performs best is: ' + str(bestSvmEpoch) +
...
r = tempTestDetails['R']
p = tempTestDetails['P']
print('Accuracy on testing the test dataset with Logistic Regression: ' + str(accuracy) +
      '% , F-Score is: ' + str(F1) +
      ', Recall is: ' + str(r) +
...
plt.ylabel('Accuracy')
plt.title('Logistic Regression: Lambda= '+str(bestLearningRateForSvm)+', Sigma Sq.='+str(bestRegParamForSvm)+',\n Best Epoch='+str(bestSvmEpoch))
plt.show()
