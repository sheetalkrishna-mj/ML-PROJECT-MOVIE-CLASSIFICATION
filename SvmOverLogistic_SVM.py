...
foldArray = [0, 1, 2, 3, 4]
seed = 0
opFileName = "SVM_Over_Logistic_Predictions.csv"
epochsForQ1 = 20
epochsForQ2 = 30
...
def init(fileName):
    return [pickle.load(open("New"+fileName+"ForSVMPickle.p", "rb")),
            pickle.load(open("pickled" + fileName + "Labels.p", "rb"))]
    ...
    if curEpoch == 1:
        ...
        for innerRate in differentLearningRates:
            ...
            for innerRegParam in differentRegParams:
                ...
                wtVector[innerRate][innerRegParam] = numpy.zeros(matrix.shape[1])
    for rowNumber in innerIndexes:
        for innerRate in differentLearningRates:
            newRate = innerRate / float(1 + curEpoch)
            for innerRegParam in differentRegParams:
                predictedLabel = wtVector[innerRate][innerRegParam].dot(numpy.array(matrix[rowNumber]).reshape(matrix.shape[1],))
                if predictedLabel * trainLbls[rowNumber] <= 1:
                    wtVector[innerRate][innerRegParam] = (1 - newRate) * wtVector[innerRate][
                        innerRegParam] + newRate * innerRegParam * trainLbls[rowNumber] * (numpy.array(matrix[rowNumber]).reshape(matrix.shape[1],))
                else:
...
def test(matrix, testLbls, wtVector, returnPredictions=0):
    ...
    if returnPredictions == 0:
        for rowNum, row in enumerate(matrix):
            for innerRate in differentLearningRates:
                for innerRegParam in differentRegParams:
                    predictedLabel = 1 if ((wtVector[innerRate][innerRegParam].dot((numpy.array(row).reshape(matrix.shape[1],)))) > 0) else -1
                    if predictedLabel == 1 and testLbls[rowNum] == 1:
                    ...
                    if predictedLabel == testLbls[rowNum]:
...
trainMatrixAndLabels = init('Train')
testMatrixAndLabels = init('Test')
splits = getSplits(int(len(trainMatrixAndLabels[1])))
...
for foldCount, fold in enumerate(combinations):
    indexForTrain = []
    for number in fold:
        indexForTrain += splits[number]
    indexesForTest = splits[list(set(foldArray).difference(set(fold)))[0]]
    weightVector = getWV(str(foldCount))
    if weightVector == {}:
        for ep in range(1, epochsForQ1 + 1):
            print("CV Epoch: " + str(ep) + "\n")
            numpy.random.seed(ep)
            numpy.random.shuffle(indexForTrain)
            weightVector = trainSVM(ep, trainMatrixAndLabels[0], trainMatrixAndLabels[1], weightVector, indexForTrain)
        pickle.dump(weightVector, open("pickledWeightVectorForFoldCombi" + str(foldCount) + ".p", "wb"))
    accuracy = test(trainMatrixAndLabels[0][indexesForTest], numpy.array(trainMatrixAndLabels[1])[indexesForTest], weightVector)
...
if os.path.isfile("pickledBestEpochStuffForSVM.p"):
    ...
    trainMatrix = trainMatrixAndLabels[0][:int(trainMatrixAndLabels[0].shape[0] - trainMatrixAndLabels[0].shape[0]/5)]
    trainLabels = numpy.array(trainMatrixAndLabels[1])[:int(trainMatrixAndLabels[0].shape[0] - trainMatrixAndLabels[0].shape[0]/5)]
    devMatrix = trainMatrixAndLabels[0][int(trainMatrixAndLabels[0].shape[0] - trainMatrixAndLabels[0].shape[0]/5):]
    devLabels = numpy.array(trainMatrixAndLabels[1])[int(trainMatrixAndLabels[0].shape[0] - trainMatrixAndLabels[0].shape[0]/5):]
    weightVector = {}
...
tempTestDetails = test(testMatrixAndLabels[0], testMatrixAndLabels[1], bestSvmClassifier)[bestLearningRateForSvm][bestRegParamForSvm]
accuracy = tempTestDetails["Accuracy"]
F1 = tempTestDetails['F1']
r = tempTestDetails['R']
p = tempTestDetails['P']
...
evalMatrixAndLabels = init('Eval')
evalMatrix = evalMatrixAndLabels[0]
evalLabels = evalMatrixAndLabels[1]
predictions = test(evalMatrix, evalLabels, bestSvmClassifier, 1)[bestLearningRateForSvm][bestRegParamForSvm][
    'Predictions']
...
plt.ylabel('Accuracy')
plt.title('SVM Over Logistic: Lambda= ' + str(bestLearningRateForSvm) + ',\n C=' + str(bestRegParamForSvm) + ', Best Epoch=' + str(
    bestSvmEpoch))
plt.show()
